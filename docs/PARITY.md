...
The walk prunes `LICENSES/` as a *directory* and never descends, so the texts
...
None of these can make comply write a *wrong* header. reuse answers `None` and
...
that *has* no comment syntax -- 36 extensions and 14 filenames, `.png` and
