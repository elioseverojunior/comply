...
crates/
  ...
  comply-mcp/                    # PLANNED -- MCP (Model Context Protocol) server
    src/
      main.rs                    # MCP server entrypoint
      tools.rs                   # Tool definitions
      server.rs                  # MCP protocol handling
  comply-lsp/                    # PLANNED -- Language Server Protocol
    src/
...
placed there would be counted by the coverage gate and never executed. It is
string adapters over `comply` and nothing else, and its `crate-type` must keep
`rlib` alongside `cdylib` or `cargo test` has nothing to link and the crate
reads as 0%.
...
- `validate_spdx(expression: &str) -> ValidationResult` -- validate an SPDX
  expression. A struct, not a thrown error: an invalid expression is an ordinary
  answer for an editor validating as the user types.
- `annotate_file(filename, source, license, copyright) -> Result<String, String>`
  -- add an SPDX header to file content. `filename` is not decoration: it selects
  the comment syntax, and content alone cannot tell a Rust file from a shell
  script -- without it `fn main() {}` came back with `#` comments. A browser has
  the name from `File.name`.
- `parse_reuse_toml(content: &str) -> Result<ParsedConfig, String>` -- parse
  REUSE.toml in-browser. Returns a summary rather than `Config`, which derives no
  `Serialize`; exposing it would mean adding serde to the core's public types to
  satisfy a binding.

`lint_project` is **deferred**. Linting discovers files by walking the
filesystem, which a browser does not have. Reaching it needs an in-memory file
source threaded through `scope` and `report` -- a change to fully covered core
code, so it belongs on its own rather than inside these bindings.

Target: `wasm32-unknown-unknown`. No filesystem access (all data supplied
from JS). Uses `wasm-bindgen` for the JS bridge.

The crate holds **no compliance logic**. Code compiled for `wasm32` never
executes under the host test harness, so logic here would be counted by the
coverage gate and never run -- the same attribution trap as an unexercised
derive, at file scale. `crate-type` keeps `rlib` beside `cdylib` so `cargo test`
has something to link on the host.
...
### Phase 4 -- WASM (partial)

- [x] `comply-wasm` -- WASM compilation target, builds for
  `wasm32-unknown-unknown`
- [x] JS API design -- `validate_spdx`, `annotate_file`, `parse_reuse_toml`
- [x] `Config::from_toml_str` -- split out of `Config::from_toml` so parsing is
  reachable without a filesystem
- [ ] `lint_project` -- blocked on an in-memory file source (see above)
- [ ] `wasm-pack` / npm packaging -- `wasm-bindgen-test` needs a browser runner
...
#### Tests build unoptimised, so the verdict does not depend on the host

`.cargo/config.toml` sets `[profile.test] opt-level = 0`, overriding the `1` that
`test` would inherit from `dev`. This is a correctness setting for the gate, not
a build-speed preference.
...
arms stay distinct blocks and attribution matches the source on both. Verified
100.00% (2257/2257) on both triples, the Linux figure through the act mirror.

A gate whose verdict depends on which machine ran it does not tell you anything,
which is why the slower test build is the right trade. Note that this is a
different failure from the two above: those were lines the tests genuinely could
not reach, this is a line the tests did reach and one backend declined to record.
...
Nothing derives a version twice. Which source is authoritative depends on what
is being asked, and conflating the two is what produced a release job that
announced `Version to publish: main`.

**GitVersion answers "what is this commit's version" -- at release time only.**
`.github/workflows/version.yml` is the single place `gittools/actions` runs; it
carries both `workflow_call` and `workflow_dispatch`, and `ci.yml` calls it as a
root job that every other job gates on. It used to run a second time inside
`release`, which meant the tool was provisioned twice and two answers existed
that a tag landing between them could make disagree.

The action publishes no `majorMinor`, so `version.yml` derives one. It builds
the object from `toJSON(steps.gitversion.outputs)` rather than a hand-written
list, capitalising the keys first: the action emits camelCase and
`gitversion /output json` emits PascalCase. GitHub expressions compare property
names case-insensitively so nothing notices, but `jq` does not -- unconverted,
`.Major` is null and `MajorMinor` composes as the literal `"null.null"`.
Capitalised, the two key sets are identical, and `mise run version:json` prints
the same object locally.

**`Cargo.toml` answers "what version do the committed files claim".** It is what
`cargo publish` uploads and what crates.io enforces, and it moves only on a
deliberate bump. GitVersion is deliberately NOT this anchor: its `SemVer`
carries `CommitsSinceVersionSource`, so it advances on every commit and a gate
against it could never converge -- the commit that sets `Cargo.toml` to
`0.0.1-2` is itself what makes the expected value `0.0.1-3`. (An amend is the one
operation that leaves the count alone; rebase, squash and cherry-pick all move
it, and this project rebases to keep `main` linear.)

`mise run version:check` enforces that every file restating the version agrees
with the anchor -- the two path dependencies in `Cargo.toml`, `docs/package.json`
and `docs/reference/cli.md`. The path dependencies are load-bearing rather than
cosmetic: `cargo publish` rejects one whose `version` disagrees with the crate it
resolves to. The list is discovered from `[workspace.dependencies]` by selecting
entries with a `path` key, so a new local crate is gated without an edit; it is
not a filesystem glob, because `comply-wasm` is a member with no dependency entry
and `mise.toml` holds nine `v0.0.1-1` strings that are examples in help text.
`version:sync` writes the fix.

**Both gates run in two places, deliberately.** `hk` selects a step by matching
its glob against the STAGED file set, and `git commit --amend` stages nothing
unless something is added -- so an amend that touches other files, or none, skips
them entirely (measured: 0 files, both steps skipped). They therefore also sit in
`ci:quick` and `pr:ready`, which ignore the staged set. `cargo:lock:check` is the
sharper case: it materialises the INDEX with `git checkout-index` and judges that,
because `audit` and `deny` run cargo first and cargo repairs a stale `Cargo.lock`
in place -- a working-tree check read a file an earlier step had already fixed and
passed while the staged lock went into the commit still stale.
