...
`github-pages-docs.yml`. Nothing reaches crates.io unless `provenance` and `sign`
both succeed first.
...
flowchart TD
    ...
    subgraph auto3["github-pages-docs.yml -- called"]
...
published by `github-pages-docs.yml` to the `gh-pages` branch, which is what
...
rustdoc through the Pages *artifact* path as well -- a repo serves from one
