...
```json
{
  ...
  "files": [
    { "path": "main.rs", "status": "OK",
      "copyrights": ["2026 Your Name"], "license": "MIT OR Apache-2.0" }
  ]
}
```
...
Add or update headers on named files.
...
`--force-dot-license` is for the other case: a *name* no built-in table claims,
...
Repair everything the configuration can already answer for.
...
Canonicalise headers that already exist.
