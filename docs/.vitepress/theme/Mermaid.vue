...
// has to expand seven of them to read one page is worse off than one who
...
// output. Deliberately NOT medium-zoom, which this component used to call here:
// its `isSupported` is `node.tagName === "IMG"`, so handing it an <svg> bound
// nothing and every click was silently ignored -- while the `cursor: zoom-in`
// affordance promised otherwise. A diagram is not an <img>, so it gets an
// overlay of its own.
...
const renderChart = async (): Promise<void> => {
  ...
  try {
    ...
    // in the markdown, so the rest of the page stays readable while it is fixed.
    failed.value = true;
    hasRendered.value = true;
  }
};
...
// listener per diagram -- seven of them on architecture.md.
