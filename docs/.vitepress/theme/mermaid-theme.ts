...
const SECTION_HUE_OFFSETS = [0, 34, -30, 18, -40, 26, -14, 40, -22, 10, -36, 30];
...
const SECTION_TONE = {
  light: { saturation: 44, lightness: 87, border: { saturation: 40, lightness: 62 } },
  dark: { saturation: 32, lightness: 28, border: { saturation: 36, lightness: 52 } },
};
 ...
 */
export const mermaidThemeVariables = (isDark: boolean): Record<string, string> => {
  ...
  const sections = Object.fromEntries(
    SECTION_HUE_OFFSETS.flatMap((offset, index) => {
      const lightness = tone.lightness + SECTION_LIGHTNESS_STEPS[index];
      return [
        [`cScale${index}`, hsl(hue + offset, tone.saturation, lightness)],
        [`cScaleBorder${index}`, hsl(hue + offset, tone.border.saturation, tone.border.lightness)],
