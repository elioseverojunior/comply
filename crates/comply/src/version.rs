...
fn parse_stamp(stamp: &str) -> Map<String, Value> {
    ...
    // `build.rs` handles it as raw text with no JSON parser. `mise run
    // version:json` deletes the same two keys, so both paths agree.
...
mod tests {
    ...
    fn the_dotnet_assembly_versions_are_dropped() {
        ...
        // Left in, `comply version --full` shows `0.0.1.0` twice beside `SemVer`
        // and `MajorMinorPatch`, reading as four disagreeing versions.
        let built = info(
