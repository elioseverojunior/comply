...
impl Config {
    ...
    pub fn from_toml(path: &Path) -> Result<Self, Error> {
        ...
        Self::from_toml_str(&content, &name)
    }

    /// Parse REUSE.toml content, with `name` used only to name the source in
    /// error messages.
    ///
    /// Split out of [`Self::from_toml`] so the parse is reachable without a
    /// filesystem -- `comply-wasm` runs on `wasm32-unknown-unknown`, where the
    /// content arrives from JavaScript and there is no path to read.
    ///
    /// # Panics
    ///
    /// Panics if `SPDX-License-Identifier` is a single-element list that produces
    /// a `None` from `Iterator::next` (should be impossible by construction).
    #[allow(clippy::missing_errors_doc)]
    pub fn from_toml_str(content: &str, name: &str) -> Result<Self, Error> {
...
"#
    ...
    #[test]
    fn parsing_from_a_string_needs_no_file() {
        // The WASM surface has no filesystem: `from_toml` cannot be reached from
        // a browser, so the parse has to be callable on content alone.
    ...
    #[test]
    fn parsing_from_a_string_names_the_manifest_in_its_error() {
        let error = Config::from_toml_str("[[annotations]]\n", "custom.toml")
            .expect_err("the required `version` key is missing");

        assert!(
            error.to_string().contains("custom.toml"),
            "the error must name the manifest it read: {error}"
        );
    }
}
// REUSE-IgnoreEnd
