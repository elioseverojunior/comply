...
///
/// The discriminants are the sort rank, ascending, so ordering a set of
/// annotations is a cast rather than a `match`. That is not a micro-
/// optimisation: the `match` this replaced lived in a small function the
/// optimiser folded away on some targets but not others, so its arms measured
/// as covered on arm64 and uncovered on the `x86_64` CI runner, and the 100%
/// gate failed on a line every test executes.
...
/// Traversal switches, each overridable from `[tool.comply.walk]`.
///
/// These were hardcoded at the one `WalkBuilder` call site. They are choices
/// about what counts as a project file, so they belong beside `ignore` in the
/// manifest rather than baked into the binary. Anything a project does not name
/// keeps the default below.
// Six bools trips `struct_excessive_bools`, which exists to discourage
// boolean-blind parameter lists. These are not parameters: each one maps 1:1
// onto a `WalkBuilder` setter of the same name, and grouping or re-typing them
// would hide that correspondence rather than clarify it.
#[allow(clippy::struct_excessive_bools)]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct WalkOptions {
    /// Skip dotfiles. Off by default -- a hidden file still needs a license.
    pub hidden: bool,
    /// Apply `.gitignore` files.
    pub git_ignore: bool,
    /// Apply the user's global excludes file.
    pub git_global: bool,
    /// Apply `.git/info/exclude`.
    pub git_exclude: bool,
    /// Only apply gitignore rules inside an actual repository.
    pub require_git: bool,
    /// Follow symlinks. Off by default -- the target is covered in its own
    /// right, and writing through a link edits a file nobody named.
    pub follow_links: bool,
}

impl WalkOptions {
    /// The built-in defaults, spelled out as a const so `ConfigBuilder::new`
    /// stays usable in const context.
...
/// Whether `haystack` already declares every identifier in `needle`.
///
/// `fix` re-reads the header it wrote, so aggregating without this check grows
/// the expression by one `AND` per run -- a file was observed carrying
/// `MIT OR Apache-2.0 AND MIT OR Apache-2.0 AND BSD-3-Clause AND BSD-3-Clause`.
///
/// Identifier sets rather than string equality, because the two sides are
/// written by different hands: `MIT AND BSD-3-Clause` already covers `MIT`,
/// which a textual comparison cannot see. When either side does not parse there
/// are no identifiers to compare, and the raw text is the only answer left.
fn already_covers(haystack: &str, needle: &str) -> bool {
    let (Ok(have), Ok(want)) = (crate::spdx::parse(haystack), crate::spdx::parse(needle)) else {
        return haystack == needle;
    };

    let have = have.identifiers();
    want.identifiers().iter().all(|id| have.contains(id))
}

/// A comment syntax a project declares in `[[tool.comply.comment-styles]]`.
///
/// One rule covers every extension or filename that shares a syntax -- eleven
/// extensions share `#` among the built-ins, so a table per key would mean
/// eleven near-identical blocks.
///
/// `close` empty means a line style. `line` is the per-line prefix inside a
/// block, which is what a C-style `/* * */` header needs and what nothing else
/// here can express.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CommentStyleRule {
    /// Extensions this rule claims, without the leading dot.
    pub extensions: Vec<String>,
    /// Exact filenames this rule claims, for files that carry no extension.
    pub filenames: Vec<String>,
    /// Opening delimiter, or the whole prefix for a line style.
    pub open: String,
    /// Prefix repeated on each header line inside a block; often empty.
    pub line: String,
    /// Closing delimiter; empty for a line style.
    pub close: String,
}
...
pub struct Config {
    ...
    /// Traversal switches from `[tool.comply.walk]`.
    pub walk: WalkOptions,
    /// Comment syntaxes from `[[tool.comply.comment-styles]]`, in parse order.
    ///
    /// Order is the identity: a `CommentStyle::Custom(i)` indexes straight into
    /// this list, which is what keeps the style `Copy`.
    pub comment_styles: Vec<CommentStyleRule>,
}
...
impl Config {
    ...
    pub fn match_annotations<'a>(&'a self, rel_path: &str) -> Vec<&'a Annotation> {
        ...
        // Override last (highest precedence), then aggregate, then closest --
        // which is the discriminant order `Precedence` declares.
        matched.sort_by_key(|a| a.precedence as u8);
        ...
        match anno.precedence {
            ...
            Precedence::Aggregate => {
                ...
                let license = match (anno_license, header_license) {
                    (Some(a), Some(h)) if !already_covers(h, &a) => Some(format!("{a} AND {h}")),
                    (Some(_), Some(h)) => Some(h.to_string()),
                    (Some(a), None) => Some(a),
...
mod tests {
    ...
    mod precedence {
        ...
        #[test]
        fn aggregate_does_not_repeat_a_licence_the_header_already_carries() {
            // `fix` re-reads the header it wrote, so an unconditional merge
            // grows the expression on every run. Observed in the wild as
            // `MIT OR Apache-2.0 AND MIT OR Apache-2.0 AND BSD-3-Clause AND
            // BSD-3-Clause`, one `AND` per invocation.
            let config = config_with(Precedence::Aggregate, &["2026 A"], "MIT");

            let r = config.resolve_licensing("src/a.rs", &[], Some("MIT AND BSD-3-Clause"));

            assert_eq!(r.license.as_deref(), Some("MIT AND BSD-3-Clause"));
        }

        #[test]
        fn aggregate_still_adds_a_licence_the_header_lacks() {
            let config = config_with(Precedence::Aggregate, &["2026 A"], "MIT");

            let r = config.resolve_licensing("src/a.rs", &[], Some("BSD-3-Clause"));

            assert_eq!(r.license.as_deref(), Some("MIT AND BSD-3-Clause"));
        }

        #[test]
        fn an_unparseable_header_expression_falls_back_to_comparing_the_text() {
            // Neither side parses, so identifier coverage cannot be computed;
            // equality of the raw strings is the only honest answer left.
            let config = config_with(Precedence::Aggregate, &["2026 A"], "MIT AND AND");

            let same = config.resolve_licensing("src/a.rs", &[], Some("MIT AND AND"));
            let other = config.resolve_licensing("src/b.rs", &[], Some("GPL-3.0-only"));

            assert_eq!(same.license.as_deref(), Some("MIT AND AND"));
            assert_eq!(
                other.license.as_deref(),
                Some("MIT AND AND AND GPL-3.0-only")
            );
        }
