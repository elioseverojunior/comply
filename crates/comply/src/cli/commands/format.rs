...
pub(crate) fn run(path: Option<&String>, files: &[String], check: bool) -> Result<bool> {
    ...
    let target_files: Vec<String> = if files.is_empty() {
        ...
        // Explicit paths skip the walk, and with it every scope rule; filter
        // them so `--files` cannot rewrite something `format` would never have
    ...
    for rel_path in &target_files {
        ...
        if style.style() == header::CommentStyle::Uncommentable {
            if check {
                eprintln!("Warning: {rel_path} has no known comment syntax, skipping");
            }
            continue;
        }
