...
//! Capture build provenance for [`comply::version`].
//!
...
//!
//! It sits in the library rather than the binary because a build script's
...
fn main() {
    ...
    // fallback here. The previous one hardcoded `-unknown-linux-gnu`, which
    // would have mislabelled every macOS and Windows build had it ever run.
    println!(
...
///
/// Mirrors `mise run version:json`, including its fallback: a machine without
/// gitversion still builds, and the version falls back to `CARGO_PKG_VERSION`.
///
